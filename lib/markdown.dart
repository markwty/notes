...
class _NotusMarkdownEncoder extends Converter<Delta, String> {
  ...
  void _writeImageTag(StringBuffer buffer, NotusAttribute<Map<String, dynamic>> string,
      {bool close: false}) {
    //final imageBytes = new File(string.value['source']).readAsBytesSync();
    //String base64Image = base64Encode(imageBytes);
    if (close) {
      //buffer.write('](data:image/png;base64, ' + base64Image + ')');
      buffer.write('](file://'+string.value['source']+')');
    } else {
      buffer.write('![Image');
    }
  }
